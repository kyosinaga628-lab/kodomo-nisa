...
MAX_RETRIES = 3
RETRY_DELAY = 5  # 秒
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "public", "fear-greed", "data.json")
...
def main():
    ...
    # 次回更新予定を計算 (JST 12:00 / 17:00 平日のみ)
    def calc_next_update(now_dt):
        update_hours = [12, 17]
        today = now_dt.date()
        for h in update_hours:
            candidate = datetime.datetime(today.year, today.month, today.day, h, 0)
            if candidate > now_dt:
                # 平日かチェック (0=月, 6=日)
                if candidate.weekday() < 5:
                    return candidate.strftime("%m月%d日 %H:%M")
        # 今日の更新がもうないので次の平日の12:00
        next_day = today + datetime.timedelta(days=1)
        while next_day.weekday() >= 5:  # 土日をスキップ
            next_day += datetime.timedelta(days=1)
        return datetime.datetime(next_day.year, next_day.month, next_day.day, 12, 0).strftime("%m月%d日 %H:%M")

    next_update = calc_next_update(now_dt)
