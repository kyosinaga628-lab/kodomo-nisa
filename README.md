...
| `auto-update-news.yml` | 毎日 7:00 JST | 金融庁・財務省・日銀の公式RSSからNISA関連の新着を検出し、`src/data/news.ts` に自動追加して直接コミット（`scripts/auto_update_news.py`）。LLM不使用・公式タイトルとリンクのみ追加するため無課金かつ誤情報リスクなし。型チェック通過時のみpush |
