    ...
    policySources: [
        {
            ...
            section: "こどもNISA（未成年者特定累積投資勘定）の創設",
        },
        {
            title: "所得税法等の一部を改正する法律",
...
export const KODOMO_NISA_FAQ: FAQItem[] = [
    {
        ...
        answer: "こどもNISA（正式名称：未成年者特定累積投資勘定）は、2027年1月から始まる0歳から17歳までのお子様を対象とした非課税投資制度です。2026年3月31日に令和8年度税制改正法が成立し、開始が確定しました。年間60万円、最大600万円まで非課税で投資でき、運用益・配当金に税金がかかりません。18歳になると自動的に成人NISAへ移行し、非課税のまま運用を継続できます。",
        answerSummary: "0〜17歳対象、年間60万円・最大600万円の非課税投資制度。2027年1月開始（確定）。",
        technicalTerms: [
    ...
    },
    {
        ...
        answer: "こどもNISAで投資できるのは、成人のつみたて投資枠と同様の商品（金融庁が定める要件を満たす投資信託）です。インデックスファンドや一部のアクティブファンドが含まれます。個別株式は対象外です。また、買付は定時・定額の積立に限定され、スポット購入はできません。",
        answerSummary: "つみたて投資枠と同じ投資信託のみ。定時・定額の積立買付に限定。個別株は対象外。",
        technicalTerms: [
    ...
    },
    {
        ...
        answer: "こどもNISAは原則として非課税での払出しができません。中学入学前は災害等やむを得ない事由がある場合のみ、中学入学年以降は子ども本人の教育費や生活費に使う場合に払出しが可能です。払出しには税務署の確認や書類提出などの手続きが必要とされています。18歳以降は成人NISAに移行するため、いつでも自由に払出しできます。",
        answerSummary: "中学入学前は災害等のみ、中学入学年以降は教育費・生活費で払出し可能、18歳以降は自由。",
        sources: ["令和8年度税制改正大綱"],
    },
    {
        ...
        answer: "旧ジュニアNISA（2023年末で新規口座開設終了）と新しいこどもNISA（2027年開始）には主に3つの違いがあります。①年間投資上限：旧80万円→新60万円、②対象商品：旧は株式も可→新はつみたて投資枠の投資信託のみ、③払出し制限：旧は18歳まで原則不可→新は中学入学年以降に教育費・生活費で使用可能。新制度は使い勝手が向上しています。",
        answerSummary: "年間上限60万円、投資信託のみ、中学入学年以降に教育費払出し可能に改善。",
        sources: ["令和8年度税制改正大綱", "金融庁「NISA特設サイト」"],
    },
    {
...
export function generateArticleSchemaForAI(articleData: {
    ...
    return {
        ...
        "publisher": {
            ...
            "logo": {
                "@type": "ImageObject",
                "url": "https://www.kodomo-nisa.jp/icon-512x512.png"
            }
        },
        "mainEntityOfPage": {
            "@type": "WebPage",
